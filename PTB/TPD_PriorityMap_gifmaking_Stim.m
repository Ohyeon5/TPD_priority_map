...
%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %For gif movie making (gif_making): initialize a cell in which the screenshots are stored
    imageArray ={};
    
    %For gif movie making (gif_making): rects for screenshots (x1 y1 x2 y2)
    gifRect = [(fxCntr(1)-400), (fxCntr(2)-100),...
        (fxCntr(1)+400), (fxCntr(2)+100)]; %Square with sidelength rect(4): gifRect = [(rect(3)-rect(4))/2, 0, ((rect(3)-rect(4))/2)+rect(4), rect(4)];
    ...
    while iTrial <= length(tt.isLeftStart) 
        ...
        %For Gif Movie making (gif_making): Save a screenshot    (after every flip)
        imageArray = [imageArray; {Screen('GetImage' , sci.wnd, gifRect)}];
        imageArray = [imageArray; {Screen('GetImage' , sci.wnd, gifRect)}];
        ...
        for iStimPres = 1:pp.nStimTot
            ...
            if tt.isiDurFrms(iTrial) > 0 && iStimPres < pp.nStimTot   %if ISI>0ms && this was not the last stim frame of the trial 
                ...
                if pp.isMask                    % mask presentation conditions
                    ...
                    %Gif Movie making (gif_making): Save a screenshot    (after every flip)
                    imageArray = [imageArray; {Screen('GetImage' , sci.wnd, gifRect)}];
    ...
    for i = 1:length(imageArray)
        ...
        if i == 1            
            imwrite(y, newmap, gifName, 'DelayTime', pp.isiDursToBeUsedFrms/pp.freqHz, 'LoopCount', LoopCount);  %,'WriteMode','overwrite');
        else
            if mod(i,2)~=0 
                %Stimulus frames
                imwrite(y, newmap, gifName, 'DelayTime', pp.stimDursToBeUsedFrms/pp.freqHz, 'WriteMode', 'append');
            else            
                if pp.isiDursToBeUsedFrms > 0
                    %ISI frames
                    imwrite(y, newmap, gifName, 'DelayTime', pp.isiDursToBeUsedFrms/pp.freqHz, 'WriteMode', 'append');
                else
                    %Stimulus frames
                    imwrite(y, newmap, gifName, 'DelayTime', pp.stimDursToBeUsedFrms/pp.freqHz, 'WriteMode', 'append');
                end
